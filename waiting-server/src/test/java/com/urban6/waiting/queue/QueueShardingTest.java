 ...
 * 그 구성에서는 이벤트 루프가 하나라 배수가 나오지 않는다. 배수는 k6/shard-probe.sh가 쟀다
 * (100만 건 기준 1대 126,743/s → 3대 371,747/s).
 */
...
class QueueShardingTest {
    ...
    void clearWindow() {
        ...
        for (int shard = 0; shard < SHARDS; shard++) {
            keys.add(QueueKeys.waiting(windowId(), shard));
            keys.add(QueueKeys.seq(windowId(), shard));
            keys.add(QueueKeys.active(windowId(), shard));
            keys.add(QueueKeys.pollDeadline(windowId(), shard));
        }
        redis.delete(keys);
    }
    ...
    void eachTokenLivesInExactlyOneShard() {
        ...
        for (Ticket ticket : tickets) {
            ...
            for (int shard = 0; shard < SHARDS; shard++) {
                Long rank = redis.opsForZSet().rank(QueueKeys.waiting(windowId(), shard), ticket.token());
                if (shard == expected) {
    ...
    void globalRankStaysWithinThePredictedError() {
        ...
        for (int i = 0; i < tickets.size(); i += 20) {
            Status status = waitingQueueService.status(tickets.get(i).windowId(), tickets.get(i).token());
    ...
    void approximationKeepsTheRankInvariants() {
        ...
        for (Ticket ticket : tickets) {
            Status status = waitingQueueService.status(ticket.windowId(), ticket.token());
            if (status.state() != State.WAITING) {
                continue;
            }
    ...
    void promotionVisitsEveryShard() {
        ...
        long activeTotal = 0;
        for (int shard = 0; shard < SHARDS; shard++) {
            Long active = redis.opsForZSet().size(QueueKeys.active(windowId(), shard));
