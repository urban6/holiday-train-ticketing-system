...
public class DailyWindow {
     ...
     * 키를 심는 쪽이 여럿(진입·승격)이라 각자 유예를 더하게 두면 같은 키에 서로 다른 TTL이 찍힌다.
     */
    public record Window(String windowId, Instant openAt, Instant closeAt) {

        /**
         * 대기 키를 마감 뒤에도 남겨 두는 시간. 마감 직전 진입자가 마감 직후에 조회해도
         * 자기 항목을 찾아야 한다. 대기 ZSet은 상한이 없어 이 시간이 곧 이전 창과 새 창이
         * 함께 메모리에 있는 중복 구간이라 짧게 잡는다.
         */
        private static final Duration WAITING_GRACE = Duration.ofMinutes(10);

        /**
         * 활성 키를 마감 뒤에도 남겨 두는 시간. 활성 ZSet은 capacity가 상한이라 오래 남겨도
         * 메모리 비용이 없다.
         *
         * <p><b>만료 체인(admission-grace + session-ttl + reservation-ttl)보다 반드시 길어야 한다.</b>
         * 멤버 score가 키 TTL을 넘으면 키가 통째로 사라져 활성 사용자 전원이 동시에 슬롯을 잃는다.
         * {@link QueueProperties}가 기동 시점에 검증한다.
         */
        static final Duration ACTIVE_GRACE = Duration.ofHours(1);

        /**
         * 순번 카운터는 대기열보다 오래 남긴다. 먼저 사라지면 카운터가 1부터 다시 시작해
         * 남아 있는 항목과 순번이 겹친다.
         */
