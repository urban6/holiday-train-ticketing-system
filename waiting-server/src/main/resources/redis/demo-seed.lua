...
--
-- enqueue.lua와 같은 seq 카운터를 쓰므로 봇과 사람이 섞여도 진입 순서가 그대로 보존된다.

local count = tonumber(ARGV[1])

-- INCR을 count번 부르는 대신 블록을 통째로 예약한다. 그래야 도는 동안 실사용자가 들어와도 순번이 겹치지 않는다.
local last = redis.call('INCRBY', KEYS[2], count)
local first = last - count + 1

-- ZADD 한 번에 몰 수도 있지만 unpack은 Lua 스택 한계(약 8000)가 있다. 청크는 호출부가 잘라 준다.
