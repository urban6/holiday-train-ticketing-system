...
기능 구현 자체보다 아키텍처 변경 전후의 성능 차이를 동일한 조건에서 비교하고 검증하는 데 목적을 두었습니다.

사용자는 대기열 진입 → 순번 조회 → 입장 → 예약 단계를 거치며, 각 단계는 다음과 같이 설계했습니다.

| 단계    | 설계 방식                                        | 설계 이유                                             |
| ------ | -------------------------------------------- | ------------------------------------------------- |
| 진입    | Kafka로 요청을 수집하고, 컨슈머가 일정 속도로 Redis 대기열에 등록   | 예매 시작 직후 수십만~수백만 건의 진입 요청이 Redis에 직접 몰리면 기존 대기자의 순번 조회까지 지연될 수 있음. 진입 요청을 Kafka에 먼저 적재해 Redis 유입률을 제어함으로써 조회가 밀려나는 폭을 제한함       |
...
- WAS 1~8대 확장 시 처리량은 17,313 → 138,548 req/s로 선형으로 증가
- WAS 1~2대 구간은 WAS CPU가 먼저 포화, 8대에서는 Redis CPU 100% 도달
- Redis 직접 실행 성능(139,912 req/s)의 99% 수준까지 도달하여 병목이 Redis임을 확인
- Kafka 도입 후 처리량 변화는 없었지만 순번 조회 p95 195.68ms → 28~32ms로 개선
- 150만 건 처리 동안 순번 유실·중복 0건
- 최종적으로 단일 Redis 기준 약 14만 req/s가 처리 한계로 확인

### 결론
- NLB와 WAS 수평 확장을 통해 처리량을 선형적으로 증가시킬 수 있었음
- WAS 병목 해소 후 Redis CPU가 100%에 도달하며 전체 시스템의 최종 병목으로 전환됨
- Kafka는 전체 처리량을 높이지는 못했지만, 진입 요청과 순번 조회 간 자원 경쟁을 완화해 조회 지연을 크게 감소시킴
- 단일 Redis 기준 약 14만 req/s가 처리 한계로 확인되었으며, 추가 처리량 확보를 위해서는 Redis 분산 구조가 필요함
...
남은 셋은 이번에 재지 못한 것들입니다. 결론을 뒤집을 값은 아니지만, 다시 잴 때 채워야 할 자리라
적어 둡니다.
