...
진입은 Kafka에 넘기고, 컨슈머가 정해진 속도로만 Redis에 넣습니다. 처리량을 올리려는 것은
아닙니다. 진입과 조회가 Redis 단일 스레드를 함께 쓰기 때문에, 개시 순간의 진입 스파이크가
Redis를 포화시키면 그 지연이 이미 줄 서 있는 사람의 순번 조회까지 번집니다. 유입률에 상한을
두면 그만큼이 폴링 몫으로 남습니다. 상한은 약 140,000 rps 중 조회가 최대 약 70,000을 쓰고
남는 60,000/s로 잡았습니다.
