...
그래서 그 스레드가 원자성의 근거이자 처리량의 상한입니다 — 명령 사이에 다른 요청이 끼어들 수 없어
Lua 스크립트가 그대로 임계 구역이 되지만, 반대로 스크립트가 길어지면 그 시간이 다른 모든 요청의
지연이 됩니다(README「배치 승격 상한 설정」).
...
이 시스템의 조회 경로가 왜 replica로 못 갈리는지, 대신 무엇으로 나눴는지는
README「Redis 인스턴스 분리 — Redis Cluster」에 있습니다.
...
이 시스템이 Redis 장애를 어떻게 다루기로 했는지는 README「Redis 장애 대응」에 있습니다.
...
```
waiting:20260728:a     ┐ 키 전체를 해싱      → 접두사가 달라 다른 슬롯
active:20260728:a      ┘
waiting:20260728:{a}   ┐ 중괄호 안쪽만 해싱  → 같은 슬롯
active:20260728:{a}    ┘
```

즉 **어떤 키들을 한 스크립트로 묶을지를 키 이름 단계에서 미리 정해 둬야 합니다.** 이 시스템은
실제로 이 방식을 씁니다 — 토큰마다 정해지는 샤드 번호를 해시 태그로 그대로 넣어
(`QueueKeys.java`) 한 대기열의 네 키를 같은 슬롯에 묶습니다. **Cluster의 자동 슬롯 배정
자체를 거스르는 게 아니라, "어떤 축으로 나눌지"만 애플리케이션이 정하고 그 뒤(어느 노드가
그 슬롯을 갖는지, failover는 누가 하는지)는 그대로 Cluster에 맡기는 조합입니다.** 자세한
내용은 README「Redis 인스턴스 분리 — Redis Cluster」에 있습니다.

## 선택 기준

| 아쉬운 것 | 가는 곳 |
| --- | --- |
| 노드가 죽으면 전부 멈춘다 | **Sentinel** 또는 **Cluster** |
| 읽기가 많다 | **Replication** + `ReadFrom` |
| 데이터가 한 대 메모리를 넘는다 | **Cluster** |
| **쓰기 처리량이 모자란다** | **Cluster + 해시 태그**(나뉘는 축은 앱이 직접 정해야) |

**마지막 줄이 가장 자주 오해되는 자리입니다.** 쓰기가 한 키에 몰리는 워크로드는 Cluster에
자연 키를 그대로 맡겨서는 나뉘지 않습니다. 노드를 늘리기 전에 **트래픽이 실제로 갈라지는 축**을
먼저 찾아야 하고, 그 축이 없으면 노드를 늘려도 바쁜 노드는 계속 하나입니다.

이 프로젝트가 쓴 축은 대기 순번 토큰입니다. 토큰마다 샤드가 정해지고(`QueueKeys.shardOf`),
그 샤드 번호를 해시 태그로 키에 넣어 Cluster에게 "이 축으로 나눠 달라"고 알려 줍니다. 축을
정하는 것은 애플리케이션의 몫이지만, 그 축을 따라 실제로 데이터를 흩고 마스터가 죽었을 때
레플리카를 승격시키는 것은 Cluster의 몫입니다 — 쓰기 확장과 고가용성을 한 번에 얻는 조합입니다.
자세한 내용과 전역 순번 근사라는 대가는 README「Redis 인스턴스 분리 — Redis Cluster」에
있습니다.
