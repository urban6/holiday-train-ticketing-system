...
#   가장 유리한 조건이라서 위 수치는 상한으로 읽어야 한다. 5초로 줄이려면 창(windowId)을 키로
#   Redis를 3대 이상 샤딩해야 하는데, 그러면 seq가 창 단위로 쪼개져 전역 순번이 깨진다.
#   다만 앱 전체가 WAS 1대에 17,700/s(README)이므로 실제 병목은 여기가 아니다 — Redis 쪽은 6~7배 여유다.
